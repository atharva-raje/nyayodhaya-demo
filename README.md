...
Run the contents of supabase/seed.sql to populate initial departments.
Go to Storage and create a public bucket named judgments.
...
High-Precision Parsing: Uses PyMuPDF to extract text and preserve page coordinates.
Structured Extraction: Gemini 2.5 Flash extracts entities (Case Number, Parties, Order Date) and identifies the responsible department.
Action Planning: Generates step-by-step compliance checklists, identifies contempt risks, and recommends strategy.
Evidence-First Citations: Every claim is anchored to a specific page and quote from the original PDF, allowing officers to verify logic instantly.
📂 Sample Data & Testing
